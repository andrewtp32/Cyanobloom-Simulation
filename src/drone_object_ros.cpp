...
void DroneObjectROS::initROSVars(ros::NodeHandle& node){
    ...
    isPosctrl = false;
    isVelMode = false;
    pubTakeOff = node.advertise<std_msgs::Empty>("/drone_0/takeoff",1024);
    pubLand = node.advertise<std_msgs::Empty>("/drone_0/land",1024);
    pubReset = node.advertise<std_msgs::Empty>("/drone_0/reset",1024);
    pubPosCtrl = node.advertise<std_msgs::Bool>("/drone_0/posctrl", 1024);
    pubCmd = node.advertise<geometry_msgs::Twist>("/drone_0/cmd_vel",1024); 
    pubVelMode = node.advertise<std_msgs::Bool>("/drone_0/vel_mode",1024);
}
