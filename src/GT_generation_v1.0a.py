...
for index, data_vector in enumerate(data_array):
    ...
    month = date[5:7]
    day = date[8:]
    # edit the "time" so that we always have the military time (e.g., 5AM -> 0500)
    edited_time = '%04.f' % int(time)
    # read the csv for the month corresponding to the weather data
    month_data = pd.read_csv(f'{base_directory}/{year}/data_{year}_{int(month)}.csv', index_col=0)
    # remove unnecessary columns in dataframe
    month_data = month_data.drop(columns=["MonthYear", "Zscore", "Year", "Month"])
    # some constant
    constant = 1 / 10000
    # find the product of the hotspot prob and the bloom appearance prob. then find the product of the hotspot prob and
    # the bloom appearance prob. then find the difference of the two. then multiply by a constant
    month_data['Hotspot Prob'] = ((month_data['Hotspot Prob'] * degree_bloom_appearance) - (
                (100 - month_data['Hotspot Prob']) * degree_bloom_disappearance)) * constant
    # add a line for the population
    month_data["Population"] = 0
    # save to a csv file
    month_data.to_csv(f'/Users/andrewphillips/Documents/Documents - Andrew’s MacBook Air/College/PaoloLab/uri_soft_wip/cyanobloom_simulation/resultant_fuzzy_and_jess_probabilities/{year}/resultant_prob_{year}-{month}-{day}_{edited_time}.csv')

    # visualize progress for the user
    print(f"Date: {date}\n"
          f"Time of Day: {edited_time}\n"
          f"Bloom appearance: {degree_bloom_appearance}\n"
          f"Bloom disappearance: {degree_bloom_disappearance}\n"
          "[" + ("/" * int(50 * index / len(data_array))) + ("." * int(50 * (1 - (index / len(data_array))))) + "]"
