...
void DroneSimpleController::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf)
{
  ...
  if (!_sdf->HasElement("robotID"))
    robotID_ = 0;
  else
    robotID_ = _sdf->GetElement("robotID")->Get<int>();

std::cout << "################################      " << robotID_ << std::endl;
  ...
  //load parameters
  cmd_normal_topic_ = ss.str() + "/cmd_vel";  // this will print out /drone_0/cmd_vel
  takeoff_topic_ = ss.str() + "/takeoff";
  land_topic_ = ss.str() + "/land";
  reset_topic_ = ss.str() + "/reset";
  posctrl_topic_ = ss.str() + "/posctrl";
  gt_topic_ = ss.str() + "/gt_pose";
  switch_mode_topic_ = ss.str() + "/vel_mode";

/*
  // load parameters
  cmd_normal_topic_ = ss.str() + "/cmd_vel";
...
*/
  ...
  if (!gt_topic_.empty()){
      pub_gt_pose_ = node_handle_->advertise<geometry_msgs::Pose>("drone_0/gt_pose",1024);    
  }
  
  pub_gt_vec_ = node_handle_->advertise<geometry_msgs::Twist>("drone_0/gt_vel", 1024);
  pub_gt_acc_ = node_handle_->advertise<geometry_msgs::Twist>("drone_0/gt_acc", 1024);
